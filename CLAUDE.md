...
## monthly-report.js — דו"ח חודשי

**דו"ח כחשבונית (`renderInvoiceReport`):**
- כפתור "דו״ח כחשבונית" מציג טבלת שורות-פריט שטוחה (לא הזמנות-כרטיס) מכל ההזמנות המסוננות כרגע (`currentFilteredOrders` — מתעדכן בכל `applySearch()`), כולל פריטי אולמות.
- שורה עם תיקון (קבלת סחורה): `item.receivedQty`/`item.actualPrice` שונים מהמוזמן — מוצג המקור עם קו חוצה, חץ, והתיקון בירוק לידו באותו תא (לא שורה נפרדת).
- התצוגה תלויה בחיפוש ספק פעיל; אם לא נבחר ספק/אין תוצאות — הכפתור מציג alert.
- `showPrompt()` / שינוי חיפוש-ספק מסתירים את הדו"ח (הוא נגזר מנתונים שעלולים להיות לא רלוונטיים יותר) — יש ללחוץ על הכפתור מחדש לאחר כל חיפוש.

**הוספת פריטים להזמנה קיימת (`loadOrderForAddition` / `?addTo=ID`):**
- טוען את **כל** האולמות של ההזמנה (ראשי + נוספים), לא רק את הראשי — כל אולם מוצג עם הפריטים הקיימים נעולים (`opacity-50`, אין כפתור מחיקה) ושורה אחת ריקה להוספה.
- `hall.lockedCount` — סופר כמה מהפריטים באולם קיימים כבר ב-DB; ב-`saveOrderAction()` רק הפריטים אחרי האינדקס הזה נשלחים כ"חדשים" ומתמזגים לתוך `existing.halls`.
- כפתורי "הסר אולם" ו"שכפל מהזמנה הראשית" **מוסתרים** לאולמות שנטענו מההזמנה הקיימת (היו מוחקים פריטים שכבר קיימים) — נשארים פעילים רק על אולם חדש שנוסף באותה עריכה.
- `orders-list.js` → `viewOrder()` מציג את כל האולמות (לא רק `order.items`) דרך `buildItemsTableHtml()` המשותפת.
